...
Basicaly it adds the following property in archived trees:
#+BEGIN_EXAMPLE
  :PROPERTIES:
...
(setq org-agenda-start-on-weekday nil)  ;;displays the agenda starting today
#+END_SRC

*** TODO Google agenda                                               :ARNAUD:
...
#+END_SRC
* Org-mode + babel:
** Seamless use of babel (no confirmation, lazy export)     :ARNAUD:ARCHIVE:
:PROPERTIES:
:header-args: :tangle no
:END:
#+BEGIN_SRC emacs-lisp
...
#+END_SRC
* Org-mode + bibtex                                                   
** Helm-bibtex configuration                                         :JULIO:
...
*** Use Mendeley as default pdfviewer with helm 
...
*** Insert org-mode citation as a link to the pdf file
This is the <f3> shortcut on helm-bibitex.

Later I would like to make this links to open in mendeley directly by
adding a new type of link like "BIB::url". So we don't store the path
in org-file, each time we want to access the link org-mode should look
on the bibtex file. 
#+BEGIN_SRC emacs-lisp
...
*** Fix the helm function to find the pdf in field                  :JULIO:
Last two lines where modified to match the syntax of the /file/ field of
