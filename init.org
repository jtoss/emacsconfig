...
#+RESULTS:
: swap-buffers-in-windows
** Recenter cursor positions in the windoe                           :JULIO:
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp
; set the prefx here doesn't work for some unkown reason ... must be set before anything else.
; (setq outline-minor-mode-prefix "\C-c \C-o") ; Or something else
...
;(add-hook 'LaTeX-mode-hook outline-minor-mode) ; latex
(setq LaTex-command "pdflatex") ; C-c C-c to run pdflatex
#+end_src

#+RESULTS:
: pdflatex

** Extra functionalities to latex-mode

Has org-mode like navigation between headers

#+begin_src emacs-lisp
(add-hook 'LaTeX-mode-hook #'latex-extra-mode)
#+end_src

** COMMENT Define keymaps
...
#+RESULTS:
: (keymap (98 . outline-backward-same-level) (102 . outline-forward-same-level) (112 . outline-previous-visible-heading) (110 . outline-next-visible-heading) (117 . outline-up-heading) (115 . show-subtree) (107 . show-branches) (105 . show-children) (101 . show-entry) (97 . show-all) (100 . hide-subtree) (108 . hide-leaves) (99 . hide-entry) (111 . hide-other) (116 . hide-body) (113 . hide-sublevels) Outline-)

* COMMENT Out-shine minor mode

Couldn't make that work

https://github.com/alphapapa/outshine#usage
...
Outshine for major modes
#+begin_src emacs-lisp
;(add-hook 'outline-minor-mode-hook 'outshine-hook-function)
;(add-hook 'emacs-lisp-mode-hook 'outline-minor-mode)  ; lisp 
;(add-hook 'LaTeX-mode-hook 'outline-minor-mode) ; latex
#+end_src
...
** Org-mode file association                                         :JULIO:

Use "xdg-open" as default commant for openning files with =C-u C-u C-x C-o=

note that =setsid= is requeried to avoid emacs killing the terminal where xdg-open started.
#+begin_src emacs-lisp
(setq org-file-apps
'((auto-mode . emacs)
     ("\\.mm\\'" . default)
...
; Causes my emacs to close randomly ... BUG ? 
; (setq org-image-actual-width nil)
#+END_SRC

#+RESULTS:
| org-ref-org-menu | org-babel-result-hide-all | org-display-inline-images | #[0 \300\301\302\303\304$\207 [add-hook change-major-mode-hook org-show-block-all append local] 5 |
...
** COMMENT EIN - Emacs Ipython Notebook
#+begin_src emacs-lisp
; (require 'ein)
#+end_src
...
#+end_src 
* COMMENT Org-mode Pandoc export
#+begin_src emacs-lisp
; (require 'ox-md nil t)
...
hook to fix image format after rst export
#+begin_src emacs-lisp
...
** Temporary fix for .org link export 
https://github.com/masayuko/ox-rst/issues/34
...
** Change default markup for =verbatim= to italic

#+begin_src emacs-lisp
(custom-set-variables
 '(org-rst-text-markup-alist
   (quote
    ((bold . "**%s**")
     (code . verb)
     (italic . "*%s*")
...
** Redefine how titles are export to .rst

Set TODO kewords as BOLD and tags ans verbatim:
...
** COMMENT Tried to fix the todo keyword with filter but it doesnt work
#+begin_src emacs-lisp
(defun fmt-todo-keyword (text backend info)
  "Keyword in bold"
  (when (org-export-derived-backend-p backend 'rst)
...
#+begin_src emacs-lisp
(replace-regexp-in-string "\\(.*\\)" "**\\1**" "TODO" )
#+end_src
